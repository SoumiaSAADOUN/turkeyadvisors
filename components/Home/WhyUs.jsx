    ...
    <div className="px-8">
      ...
      <div className="flex gap-4 flex-wrap">
        {values.map((value, index) => (
          <div
            key={`value-${index}`}
            className="flex px-8 py-4 my-12 cursor-pointer gap-8 flex-col rounded-xl border border-LIGHT_GREY basis-1/5 grow  group hover:bg-[0% -200%] bg-[0% 0%] bg-size[100% 200%] hover:transition-color transition-linear transition duration-700 ease bg-gradient-to-r from-BACKGOUND_COLOR to-BACKGOUND_COLOR  hover:bg-PRIMARY_COLOR "
          >
            ...
            <h4 className="text-xl font-bold text-PRIMARY_COLOR group-hover:text-SECONDARY_COLOR">
              {value.title}
            </h4>
            <p className="text-PRIMARY_COLOR group-hover:text-SECONDARY_COLOR">
              {value.text}
            </p>
          </div>
        ))}
